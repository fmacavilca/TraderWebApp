...
The easiest way to get started with Blazor Hero is to install the [NuGet package](https://www.nuget.org/packages/BlazorHero.TraderWebApp/) and run `dotnet new BlazorHero.TraderWebApp`:
...
4. Open up Command Prompt and run `dotnet new --install BlazorHero.TraderWebApp` to install the project template
5. Create a folder for your solution and cd into it (the template will use it as project name)
6. Run `dotnet new BlazorHero.TraderWebApp` to create a new Solution with all the Awesomeness 🕶️ of BlazorHero 🦸
